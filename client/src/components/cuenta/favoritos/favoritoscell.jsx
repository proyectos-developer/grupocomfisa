...
export default function FavoritosCell({proporcional}) {
    ...
    return (
        <div className='rounded' style={{width: 459 / proporcional, height: 'auto', border: '1px solid #384da7', marginLeft: 20 / proporcional, marginRight: 20 / proporcional}}>
            {
                lista_favoritos && lista_favoritos.length > 0 ? (
                    lista_favoritos.map ((favorito, index) => {
                        return (
                            <CardFavoritoCell proporcional={proporcional} favorito={favorito} total={lista_favoritos.length - 1} index={index}/>
                        )
                    })
                ) : null
            }

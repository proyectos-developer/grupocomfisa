...
export default function Favoritos({proporcional}) {
    ...
    return (
        <div className='rounded' style={{width: 600 / proporcional, height: 'auto', border: '1px solid #384da7'}}>
            {
                lista_favoritos && lista_favoritos.length > 0 ? (
                    lista_favoritos.map ((favorito, index) => {
                        return (
                            <CardFavorito proporcional={proporcional} favorito={favorito} total={lista_favoritos.length - 1} index={index}/>
                        )
                    })
                ) : null
            }

...
import CardProductoCotizacionCell from './card/productocotizacioncell.jsx'
...
export default function ListaProductosCell({proporcional}) {
    ...
    return (
        <div className='rounded' style={{width: '100%', height: 'auto', paddingLeft: 20 / proporcional, paddingRight: 20 / proporcional}}>
            {
                lista_productos && lista_productos.length > 0 ? (
                    lista_productos.map ((producto, index) => {
                        return (
                            <CardProductoCotizacionCell proporcional={proporcional} producto={producto} total={lista_productos.length - 1} index={index}/>
                        )
                    })
                ) : null
            }

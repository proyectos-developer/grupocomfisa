...
import CardProductoCotizacion from './card/productocotizacion.jsx'
...
export default function ListaProductos({proporcional}) {
    ...
    return (
        <div className='rounded' style={{width: 900 / proporcional, height: 'auto'}}>
            {
                lista_productos && lista_productos.length > 0 ? (
                    lista_productos.map ((producto, index) => {
                        return (
                            <CardProductoCotizacion proporcional={proporcional} producto={producto} total={lista_productos.length - 1} index={index}/>
                        )
                    })
                ) : null
            }

...
import CardProductoResultadoTablet from './card/productoresultadotablet.jsx'
...
export default function ProductosTablet({proporcional}) {
    ...
    return (
        <div className='position-relative' style={{width: '100%', paddingTop: 60 / proporcional, paddingBotton: 60 / proporcional, paddingRight: 60 / proporcional,
                paddingLeft: 60 / proporcional}}>
                ...
                lista_productos && lista_productos.length > 0 ? ( 
                    lista_productos.map ((producto, numprod) => {
                        return (
                            ...
                            {
                                productos[(2 *  numprod)] ? ( 
                                    <CardProductoResultadoTablet producto={productos[(2 *  numprod)]} key={(2 *  numprod)} index={(2 *  numprod)} proporcional={proporcional}/>
                                ) : null
                            }
                            {
                                productos[(2 *  numprod) + 1] ? ( 
                                    <CardProductoResultadoTablet producto={productos[(2 *  numprod) + 1]} key={(2 *  numprod) + 1} index={(2 *  numprod) + 1} proporcional={proporcional}/>
                                ) : null
                            }
                            </div>
                        )

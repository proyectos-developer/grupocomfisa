...
export default function ListaProductosProveedor({proporcional}) {
    ...
    return (
        <div className='position-relative' style={{width: '100%', paddingLeft: 350 / proporcional, paddingRight: 350 / proporcional, paddingTop: 20 / proporcional}}>
                ...
                lista_productos && lista_productos.length > 0 ? ( 
                    lista_productos.map ((producto, numprod) => {
                        return (
                            ...
                            {
                                productos[(4 *  numprod)] ? ( 
                                    <CardProductoLista producto={productos[(4 *  numprod)]} key={(4 *  numprod)} index={(4 *  numprod)} proporcional={proporcional}/>
                                ) : null
                            }
                            {
                                productos[(4 *  numprod) + 1] ? ( 
                                    <CardProductoLista producto={productos[(4 *  numprod) + 1]} key={(4 *  numprod) + 1} index={(4 *  numprod) + 1} proporcional={proporcional}/>
                                ) : null
                            }
                            {
                                productos[(4 *  numprod) + 2] ? ( 
                                    <CardProductoLista producto={productos[(4 *  numprod) + 2]} key={(4 *  numprod) + 2} index={(4 *  numprod) + 2} proporcional={proporcional}/>
                                ) : null
                            }
                            {
                                productos[(4 *  numprod) + 3] ? ( 
                                    <CardProductoLista producto={productos[(4 *  numprod) + 3]} key={(4 *  numprod) + 3} index={(4 *  numprod) + 3} proporcional={proporcional}/>
                                ) : null
                            }
                            </div>
                        )

...
export default function CardProductoInformacionTablet({producto, proporcional}) {
    ...
    useEffect(() => {
        if (get_producto && get_producto.success === true && get_producto.producto){
            ...
            dispatch (set_lista_carrito_cotizacion(new_cotizar.cotizar))
            navigate (`/proveedor/${get_producto.producto.proveedor.replace (' ', '-')}/producto/${get_producto.producto.id}`)
            window.scrollTo(0, 0)
        }
    }, [get_producto])
    ...
    return (
        <div className='shadow rounded' style={{width: 415.5 / proporcional, marginRight: 10 / proporcional, marginLeft: 10 / proporcional, marginBottom: 40 / proporcional}}
